...
export function LiteApp() {
  ...
  return <UiPreferencesContext.Provider value={preferences}><ConfirmProvider><main className={styles.app} data-runtime="lite" data-session-epoch={session.sessionEpoch} data-route={route.panel}>
    ...
    {route.panel === 'home' && <section className={styles.panel} aria-label="Главная"><LiteHome journey={session.data.journey} scores={session.data.scores} diaryCount={session.data.diary.length} fallbackNotice={route.fallbackNotice} onNavigate={openPanel} onUnavailable={openUnavailable} /></section>}
