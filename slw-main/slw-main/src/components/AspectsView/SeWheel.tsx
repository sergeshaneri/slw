...
type QuadrantKey = 'defender' | 'ruler' | 'builder' | 'hero'

const QUADRANT_ORDER: QuadrantKey[] = ['defender', 'ruler', 'builder', 'hero']
...
export default function SeWheel({ skills, color, onContinueSurveys, isLocked = false }: Props) {
  ...
  const archeStates = QUADRANT_ORDER.map((key, qi) => {
    ...
    const avg = calcSeArchetypeAvg(skillsArg, key)
      ...
      <header className={styles.wheelHeader}>
        ...
        <div className={styles.wheelStat}>
          <div className={styles.wheelStatVal}>
            {Number.isFinite(seScore) ? (seScore as number).toFixed(1) : '—'}
