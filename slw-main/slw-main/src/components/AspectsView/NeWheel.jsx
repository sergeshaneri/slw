...
const QUADRANT_ORDER = ['sage', 'pioneer', 'catalyst', 'visionary']
...
export default function NeWheel({ skills, color, onContinueSurveys, isLocked = false }) {
  const neScore = calcNeScoreFromSkills(skills)
  ...
  const archeStates = QUADRANT_ORDER.map((key, qi) => {
    ...
    const avg = calcNeArchetypeAvg(skills, key)
      ...
      <header className={styles.wheelHeader}>
        ...
        <div className={styles.wheelStat}>
          <div className={styles.wheelStatVal}>
            {Number.isFinite(neScore) ? neScore.toFixed(1) : '—'}
