...
const QUADRANT_ORDER = ['analyst', 'architect', 'guardian', 'encyclopedist']
...
export default function TiWheel({ skills, color, onContinueSurveys, isLocked = false }) {
  const tiScore = calcTiScoreFromSkills(skills)
  const progress = getTiSkillProgress(skills)
  ...
  const archeStates = QUADRANT_ORDER.map((key, qi) => {
    ...
    const avg = calcTiArchetypeAvg(skills, key)
      ...
      <header className={styles.wheelHeader}>
        ...
        <div className={styles.wheelStat}>
          <div className={styles.wheelStatVal}>
            {Number.isFinite(tiScore) ? tiScore.toFixed(1) : '—'}
