// Парсер анкет навыков БС.
// Источник — `surveys.md` (копия исходника `Si/вопросы для оценки навыков БС.md`).
//
// Формат:
...
export function parseSurveys(md) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const archetype = SKILL_TO_ARCHETYPE[id]
    if (!archetype) {
      // eslint-disable-next-line no-console
