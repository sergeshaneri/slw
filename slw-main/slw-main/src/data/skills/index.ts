...
export function getSkillContent(skillId: string): Skill | null {
...
export function hasSkillContent(skillId: string): boolean {
...
export function getSkillName(skillId: string): string {
  ...
  if (siKey) {
    const skill = (SI_SKILL_TREE[siKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const feKey = FE_SKILL_TO_ARCHETYPE[skillId]
  if (feKey) {
    const skill = (FE_SKILL_TREE[feKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const neKey = NE_SKILL_TO_ARCHETYPE[skillId]
  if (neKey) {
    const skill = (NE_SKILL_TREE[neKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const niKey = NI_SKILL_TO_ARCHETYPE[skillId]
  if (niKey) {
    const skill = (NI_SKILL_TREE[niKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const teKey = TE_SKILL_TO_ARCHETYPE[skillId]
  if (teKey) {
    const skill = (TE_SKILL_TREE[teKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const tiKey = TI_SKILL_TO_ARCHETYPE[skillId]
  if (tiKey) {
    const skill = (TI_SKILL_TREE[tiKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const fiKey = FI_SKILL_TO_ARCHETYPE[skillId]
  if (fiKey) {
    const skill = (FI_SKILL_TREE[fiKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  const seKey = SE_SKILL_TO_ARCHETYPE[skillId]
  if (seKey) {
    const skill = (SE_SKILL_TREE[seKey] ?? []).find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (SI_COMMON_BASE_IDS?.has(skillId)) {
    const skill = SI_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (FE_COMMON_BASE_IDS?.has(skillId)) {
    const skill = FE_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (NE_COMMON_BASE_IDS?.has(skillId)) {
    const skill = NE_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (NI_COMMON_BASE_IDS?.has(skillId)) {
    const skill = NI_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (TE_COMMON_BASE_IDS?.has(skillId)) {
    const skill = TE_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (TI_COMMON_BASE_IDS?.has(skillId)) {
    const skill = TI_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (FI_COMMON_BASE_IDS?.has(skillId)) {
    const skill = FI_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }

  if (SE_COMMON_BASE_IDS?.has(skillId)) {
    const skill = SE_COMMON_BASE_SKILLS.find((s: { id: string; name?: string }) => s.id === skillId)
    if (skill?.name) return skill.name
  }
...
export function getArchetypeNameForSkill(skillId: string): string | null {
  const siKey = SI_SKILL_TO_ARCHETYPE[skillId]
